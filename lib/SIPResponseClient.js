...
class SIPResponseClient extends SIPBaseClient {
  ...
  registered (bindings, params) {
    ...
    const message = new Registered(...arguments);
    if (message) {
  ...
  unregistered (bindings, params) {
    ...
    const message = new Unregistered(...arguments);
    if (message) {
      return this.send(message);
    }
  }

  callList (calls, params) {
    if (!calls || !Array.isArray(calls)) {
      throw new Error('Error: invalid calls array');
    }

    const message = new Calls(calls, params);
    if (message) {
      return this.send(message);
    }
  }

  callInfo (info, params) {
    if (!info || typeof info !== 'object') {
      throw new Error('Error: invalid callInfo object');
    }

    const message = new CallInfo(info, params);
    if (message) {
      return this.send(message);
    }
  }
  ...
  error (response, params) {
    ...
    const message = new ErrorResponse(response, params);
