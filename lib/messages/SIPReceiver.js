...
class SIPReceiver extends EventEmitter {
  ...
  handleMessage(message) {
    ...
    if ( pMessage && typeof(pMessage) === 'object' &&
      ...
      if (pMessage.name === C.ERROR) {
        SIPTransactionManager.rejectTransaction(transactionId, pMessage.body);
      } else {
        SIPTransactionManager.resolveTransaction(transactionId, pMessage.body);
      }

      const payload = Object.assign({ transactionId }, pMessage.body);
      _self.emit('api', pMessage.name, payload);
    }
  }
}
